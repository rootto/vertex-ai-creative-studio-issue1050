...
If you are unable to create a DNS record in your corporate domain, you can use the autogenerated Cloud Run domain. The application will be publicly accessible at the infrastructure level but secured at the application level using **Identity Platform**.
...
### 2. Configure Identity Platform and OAuth Client (Manual)

Since Terraform cannot fully configure Identity Platform due to environment quota restrictions, and the OAuth client requires the Cloud Run URL, you must perform this setup manually:

1.  In the Google Cloud Console, ensure you are in your project.
...
Now that the infrastructure is ready and Identity Platform is configured, build and deploy the application:
