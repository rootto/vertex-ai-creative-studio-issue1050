...
If you are unable to create a DNS record in your corporate domain, you can use the autogenerated Cloud Run domain. The application is secured natively at the infrastructure level using **Identity-Aware Proxy (IAP)**.
...
Make sure to take note of the **Cloud Run URL** that is output (e.g., `https://creative-studio-863507536998.us-central1.run.app`).

### 2. Build and Deploy Container Image

Now that the infrastructure is ready, build and deploy the application:

```bash
./build.sh
```

Congratulations, you can now navigate to the address provided in the `cloud-run-app-url` Terraform output. Access to this URL is automatically restricted to the `initial_user` via native Cloud Run IAP.
...
- **GCS Media Access & Auto-Refreshing Signed URLs**: To prevent GCS media links (which are signed for security) from expiring after 15 minutes while the user is using the app, the application routes all GCS asset displays through a secure `/media/` proxy path. When the browser requests media from this path, the backend verifies the user's IAP identity, generates a fresh, short-lived signed GCS URL, and returns an HTTP 307 temporary redirect. This ensures GCS access never expires while keeping the GCS bucket private.
