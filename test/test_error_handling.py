...
def test_get_safety_reason_known_code():
    ...
    assert (
        result
        == "Safety Filter Blocked: Celebrity (Support Code: 15236754). Rejects requests to generate a photorealistic representation of a prominent person or if the project isn't on the allowlist for this feature. Please adjust your request."
    )
...
def test_get_safety_reason_unknown_code():
    ...
    assert (
        result
        == "Safety Filter Blocked: Unknown Safety Issue (Support Code: 99999999). No additional details available. Please adjust your request."
    )
...
def test_get_safety_reason_code_syntax():
    ...
    assert (
        result
        == "Safety Filter Blocked: Child (Support Code: 58061214). Rejects requests to generate content depicting children if personGeneration isn't set to 'allow_all' or if the project isn't on the allowlist for this feature. Please adjust your request."
    )
