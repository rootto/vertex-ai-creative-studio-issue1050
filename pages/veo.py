...
from config.veo_models import DEFAULT_VEO_VERSION_ID, get_veo_model_config
...
def on_click_extend_video(e: me.ClickEvent):
    ...
    request = VideoGenerationRequest(
        ...
        duration_seconds=state.video_extend_length
        if state.video_extend_length != 0
        else (
            model_config.supported_extension_durations[0]
            if model_config.supported_extension_durations
            else 7
        ),  # Use extension length or default
...
def on_click_veo(e: me.ClickEvent):  # pylint: disable=unused-argument
    ...
    try:
        base_url = (
            "http://localhost:8080"
            if "localhost" not in config.API_BASE_URL
            else config.API_BASE_URL
        )
