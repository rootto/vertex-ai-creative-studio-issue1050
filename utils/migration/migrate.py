...
# Disable mTLS checking to prevent MutualTLSChannelError when pyOpenSSL is disabled/missing
os.environ["GOOGLE_API_USE_CLIENT_CERTIFICATE"] = "false"

# Force python to use standard library ssl instead of pyOpenSSL to avoid SSL context mutation bugs in urllib3
...
def copy_gcs_object(
    ...
    try:
        ...
        if not src_blob.exists():
            print(
                f"  [WARN] Source GCS file does not exist: gs://{src_bucket_name}/{relative_path}",
            )
            STATS["errors"] += 1
            return False
        ...
        if dest_blob.exists():
            print(
                f"  [SKIP] Destination GCS file already exists: gs://{dest_bucket_name}/{relative_path}",
            )
            STATS["files_skipped"] += 1
            return True

        print(
            f"  [COPY] gs://{src_bucket_name}/{relative_path} -> gs://{dest_bucket_name}/{relative_path}",
        )
        if not dry_run:
...
def migrate_collection(
    ...
    print(f"\nMigrating collection: {source_col_ref.id} -> {dest_col_ref.id}")
    ...
    for doc in docs:
        ...
        if dest_doc_ref.get().exists and not force:
            print(
                f"  [SKIP] Document '{doc.id}' already exists at destination (use --force to overwrite)",
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--source-bucket",
        required=True,
        help="Source GCS assets bucket name",
    )
    parser.add_argument(
        "--dest-bucket",
        required=True,
        help="Destination GCS assets bucket name",
    )
    parser.add_argument(
        "--source-db",
        default="(default)",
        help="Source Firestore database name (default: '(default)')",
    )
    parser.add_argument(
        "--dest-db",
        default="(default)",
        help="Destination Firestore database name (default: '(default)')",
    )
    parser.add_argument(
        "--source-credentials",
        help="Path to source service account key JSON file",
    )
    parser.add_argument(
        "--dest-credentials",
        help="Path to destination service account key JSON file",
    )
    parser.add_argument(
        "--collection",
        default="genmedia",
        help="Root collection to migrate (default: 'genmedia')",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Log migration actions without modifying files or database",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing Firestore documents and force copying GCS assets",
    )
    ...
    try:
        ...
        db_source = get_firestore_client(
            args.source_project,
            args.source_db,
            args.source_credentials,
        )
        db_dest = get_firestore_client(
            args.dest_project,
            args.dest_db,
            args.dest_credentials,
        )

        storage_source = get_storage_client(
            args.source_project,
            args.source_credentials,
        )
